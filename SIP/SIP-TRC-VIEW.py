...
def file_handler(dump_log):
	global tail_chunk
	global flag_log

	f = subprocess.Popen(['tail','-F',dump_log],stdout=subprocess.PIPE,stderr=subprocess.PIPE)

	while True:
		line = f.stdout.readline()

		tail_chunk.append(line)
		separator = '\\n'

		if line == separator:
#			print "callflow start!!!!!!!! ------------------------------------------------"
			show_callflows(tail_chunk)
			if flag_log:
				show_calllogs(tail_chunk)
			tail_chunk = []

class Paragraphs:
	def __init__(self, dump_log, separator='\\n'):
		self.seq = dump_log
	...
	def __getitem__(self, index):
		...
		dirc = re.findall('RECV|SEND|Recv|Send',line)
		if 'SEND' in dirc or 'Send' in dirc:
			self.flow_dirc = '|<----XXXXXXXXXXXXXX-----|'
		elif 'RECV' in dirc or 'Recv' in dirc:
			self.flow_dirc = '|-----XXXXXXXXXXXXXX---->|'
...
def show_callflows(dump_log):
	pp = Paragraphs(dump_log)
	global flow_number
...
def main():
	...
	dump_log = '/home/brbt/log/sipa_dump.' + date

	try:
		file_handler(dump_log)
