...
class HealthCheckCommand(object):
	...
	def GetHaStatus(self,VirtualNetworkInterface):
		NetworkInterfaceNameList = self.GetNetworkInterface()
		for Iface in VirtualNetworkInterface:
			if Iface in NetworkInterfaceNameList :
				HaStatus = 'ACTIVE'
				return HaStatus
			else:
				HaStatus = 'STANDBY'
...
#           	print(" family :  : %s" % addr.family)
	...
	def ha_status(self, VIRTUAL_IP):
		IPs = self.IP_address_List()

		if VIRTUAL_IP in IPs :
			HaStatus = 'ACTIVE'
			return HaStatus
		else:
			HaStatus = 'STANDBY'
	...
	def hostname(self):
		...
		if 'HostName' in locals():
			logger.info('%s : hostname is %s', self.GetCurFunc(), HostName)
			return HostName   # SPS01, MPS01, SS701
		else:
			logger.info('%s :: %s is unknown IP address, check HostConf List',self.GetCurFunc(), LocalIpAddress)
			sys.exit()
	...
	def GetHostName(self,IP_ADDR):
		LocalIpAddress = self.GetIP(IP_ADDR)
		logger.debug('%s : Local IP Address : %s', self.GetCurFunc(), LocalIpAddress)
		if LocalIpAddress is None:
			logger.info('%s :: %s is not configured IP address', self.GetCurFunc(), IP_ADDR)
			logger.info('%s :: sys.exit()', self.GetCurFunc())
			sys.exit()
		for host in HostConf:
			if LocalIpAddress in host:
				HostName = host[1]
