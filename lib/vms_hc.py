...
class HostInfo(object):
   ...
   def __init__(self):
      ...
      if search_flag == 0:
         print "¿Â∫Ò¿« IP¡÷º“∞° hostconf.pyø° µÓ∑œµ«æÓ ¿÷¡ˆ æ Ω¿¥œ¥Ÿ."
         print "º≥¡§¿ª »Æ¿Œ«œººø‰."
         sys.exit()
...
def disk_mirror_status():
   ...
   if SYS_MANUF == 'HP':
      ...
      os_command = "SKIP (Console Cable ø¨∞· « ø‰)"
...
def vms_subscribers():
   vms_subscribers_sps = OdbcQuery('º“∏Æª˘ √—∞°¿‘¿⁄','altibase')
...
def vic_subscribers():
   vic_subscribers_sps = OdbcQuery('VIC √—∞°¿‘¿⁄','altibase')
...
def isup_stat():
   isup_stat_omp = OdbcQuery('ISUP ≈Î∞Ë »Æ¿Œ','mysql')
   isup_stat_omp.db_column_name=['≈Î∞ËΩ√∞£','IAM','CPS','ªÁøÎ∑Æ(%)']
...
def scp_stat():
   scp_stat_omp = OdbcQuery('SCP ≈Î∞Ë »Æ¿Œ','mysql')
   scp_stat_omp.db_column_name=['≈Î∞ËΩ√∞£','TotalCount','CPS','ªÁøÎ∑Æ(%)/250 CPS']
...
def cpu_stat():
   cpu_stat_omp = OdbcQuery('CPU ≈Î∞Ë»Æ¿Œ','mysql')
   cpu_stat_omp.db_column_name = ['Ω√Ω∫≈€','Peak ∞™','∆Ú±’∞™']
...
def tars_sip_stat():
   sip_stat_omp = OdbcQuery('SIP ≈Î∞Ë »Æ¿Œ','mysql')
   sip_stat_omp.db_column_name=['≈Î∞Ëºˆ¡˝Ω√∞£','TOTAL','SUCCESS','FAIL','HT']
...
#   print("æÀ∂˜ ¡∂»∏ ±‚∞£ : %s ~ %s " % ( alarm_start_time, alarm_end_time ))
#   print("¿€æ˜Ω√∞£(02:00 ~ 05:59) æÀ∂˜¿∫ ¡¶ø‹")
...
def sip_stat():
   sip_stat_omp = OdbcQuery('SIP ≈Î∞Ë »Æ¿Œ', 'mysql')
   sip_stat_omp.db_column_name=['≈Î∞ËΩ√∞£','TotalCount','CPS','ªÁøÎ∑Æ(%)/300 CPS']
...
def altibase_tablespace():
   altibase_tablespace_sps = OdbcQuery('æÀ∆º∫£¿ÃΩ∫ ∏ﬁ∏∏Æ ªÁøÎ∑¸', 'altibase')
...
def ping_status():
   ...
   hc_result_table = HcCmdResultTalbe('Ping »Æ¿Œ : ping -c 4 -w 4 gateway_ip',78)
...
def route_status():
   ...
   hc_result_table = HcCmdResultTalbe('route »Æ¿Œ : ping -c 4 -w 4 gateway_ip',78)
   ...
   route_result += "Netconf∆ƒ¿œ¡§∫∏¿Øπ´               ∫Ò∞Ì\n"
   ...
   for route_dest_section in route_dest_sections_from_config:
      ...
      if search_result != "OK":
         ...
         route_result += ' routing tableø° æ¯¿Ω\n'
         route_status_result.result = "NOK"
...
def crontab_status():
   ...
   for crontab_entry,crontab_command in crontab_from_config:
      ...
      if search_result != "OK":
         ...
         crontab_result += ' crontab ∏Ò∑œø° æ¯¿Ω\n'
